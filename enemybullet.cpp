...
static char* g_TexturName[] = {
	"data/TEXTURE/enemy_bullet.png",
};

static BOOL			g_Load = FALSE;			// 初期化を行ったかのフラグ
static ENEMYBULLET	g_EnemyBullet[ENEMYBULLET_MAX];	// バレット構造体
...
HRESULT InitEnemyBullet(void)
{
	...
	for (int i = 0; i < ENEMYBULLET_MAX; i++)
	{
		...
		g_EnemyBullet[i].move = XMFLOAT3(-ENEMYBULLET_SPEED, 0.0f, 0.0f);	// 移動量を初期化
	}
...
void UpdateEnemyBullet(void)
{
	for (int i = 0; i < ENEMYBULLET_MAX; i++)
	{
		int EnemyBulletCount = 0;            // 処理したバレットの数
